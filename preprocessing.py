...
def main():
    ...
    if not os.path.exists("output/df.pkl"):
        path = 'dataset/BC7-LitCovid-Train.csv'
        ...
        for index, row in enumerate(df['label']):
            if ";" in row:
                ...
                for i in range(1, len(arr)):
                    ...
                    mega_index += 1
        write_file = open("output/df.pkl", "wb")
        pickle.dump(df, write_file)
    else:
        df = pickle.load(open("output/df.pkl", "rb"))

    if not os.path.exists("output/title_abstract_word_frequency.pkl"):
        #encoding the labels
        print("encode label")
        ...
        write_file = open("output/title_abstract_word_frequency.pkl", "wb")
        pickle.dump(title_abstract_word_frequency, write_file)
    else:
         title_abstract_word_frequency = pickle.load(open("output/title_abstract_word_frequency.pkl", "rb"))
    if not os.path.exists("output/new_tokens.pkl"):
        x = Counter(tokens)
        for word, count in x.items():
            if count > 20:
                bag_of_words.add(word)
        write_file = open("output/new_tokens.pkl", "wb")
    ...
    write_file = open("output/train_X.pkl", "wb")
    pickle.dump(dx, write_file)

    write_file = open("output/train_Y.pkl", "wb")
